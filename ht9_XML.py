...
""")
    print("""          XML 
The source XML file should be in .xml extension and contains data in the following format: 
<root> 
...
Each <publication> tag is a separate post and must contain attribute name = 'News', 'PrivateAd' or 'Recipe'. 
Each publication has to contain <body> and <foot> as child tags for publication text and footer respectively. 
...
-------------------------------------------------------------------------------------------------------------- 
           ...
           JSON 
The source JSON file should be in .json extension and contains data in following format: 
One publication: 
...
-------------------------------------------------------------------------------------------------------------- 
            ...
            TXT 
The source txt file should be in .txt extension and contains data in the following format: 
News--News text--City 
...
class Operator():
    def start(self):
        choice = input("""Do you want to make publication by console input, from TXT file input, JSON input or XML input?
C for Console input\nF for from txt File\nJ for Json file input\nX for Xml file input\nI to view Info
S for change Settings\nE for Exit the program\n""").lower()
        if choice not in ['c', 'f', 'i', 's', 'j', 'x','e']:
...
P to Proceed without any changes\nE for exit the program\n""").lower()
        if setting not in ['o', 's', 'p','j', 'x', 'e']:
            ...
            if setting == 'o':
                ...
                proceed = input('Do You also want to change path for output publications?\nY/N\n').lower()
                if proceed == 'y':
...
Program ended""")
    ...
    def xml_publish(self):
        if xml_source_path.endswith('.xml'):
            try:
                xml_file = ET.parse(xml_source_path)
                root = xml_file.getroot()

                for publication in root:
                    body_element = iz_replace(text_capitalazing(str(publication.find('body').text)))
                    foot_element = publication.find('foot').text
                    if 'None' in [body_element, foot_element]:
                        print(no_empty_space_head_body_foot)
                ...
                return print("""Invalid format of XML file content. Please reload the program.
Read the Info and change the content of the file.       
Alternatively, you can also select a different file in the settings.\nProgram ended """)
            ...
            return print(f"""File {xml_source_path} should have .xml extension.
Please reload the program. And change the file extension or choose another file in the settings.\n
Program ended""")
